...
touch "$server_config" 2>/dev/null || { entrypoint_log "ERROR: cannot modify $server_config(read-only filesystem?)"; exit 1; }
...
sed -i.bak -r 's/^(worker_processes)(.*)$/# Commented out by '"$SELF"' on '"$(date)"'\n#\1\2\n\1 '"$ncpu"';/' "$server_config"
